...
def maybe_adjust_costs_and_potentials(n, opts):
    ...
    for o in opts:
        if "+" not in o: continue#serves to skip things that do not have a plus
        oo = o.split("+")
        ...
        suptechs = map(lambda c: c.split("-", 2)[0], carrier_list)#what does this line do
        if oo[0].startswith(tuple(suptechs)):
            carrier = oo[0]
            attr_lookup = {"p": "p_nom_max", "e": "e_nom_max", "c": "capital_cost"}
            attr = attr_lookup[oo[1][0]]#first character in the second item created by split
            factor = float(oo[1][1:])
...
if __name__ == "__main__":
    ...
    network_name = snakemake.wildcards.sector_opts + '-solar+c' + snakemake.wildcards.solar_cost

    opts = network_name.split('-')
