...
from matplotlib.patches import Polygon #canvasas with a face color and edge color
from matplotlib.collections import PatchCollection #to assign colors to patches
...
cmap = cm.get_cmap('gist_ncar', 20)  # matplotlib color palette name, n colors. 20 different colors
...
for c in Europe_ia3.keys():
    ...
    m_plot.readshapefile('shapefiles/gadm40_' + Europe_ia3[c] + "_shp/gadm40_" + Europe_ia3[c] + "_0",c,drawbounds=True,linewidth = 0,color='k')
    patches = []
    value = gas_rel
    for info, shape in zip(eval('m_plot.' + c + '_info'), eval('m_plot.' + c)):
        patches.append(Polygon(np.array(shape), True))
    patch1=ax.add_collection(PatchCollection(patches, facecolor= cmap(norm(value))))#facecolor based on value of gas
    
    # ax.annotate(CC_plot.Code,xy=m_plot(np.array(CC_plot.Lon.item()-1), np.array(CC_plot.Lat.item())),color='k', zorder=13)
    x,y=m_plot(np.array(CC_plot.Lon.item()-1), np.array(CC_plot.Lat.item())) #This plots the gas magenta dots for each country
    m_plot.plot(x,y,"o",markersize=gas_abs/max_val*50,color='magenta',zorder=100)#zorder sets the order of plotted things. 
    #zorder = 100 means that we could fit 99 things before, but this would never happen anyway. 
    # It just puts it last
...
if arrows == True:
    ...
    fig.savefig('../figures/Gas_' + case + '_map_with_arrows2.jpeg',format='jpeg',dpi=300,bbox_inches='tight')  

else:
    fig.savefig('../figures/Gas_' + case + '_map_wo_arrows5.jpeg',format='jpeg',dpi=300,bbox_inches='tight')  
