...
import csv
import glob
from helperfunctions import pen_plus_any_curtailoverlap_t, pen_plus_any_poster_curtailoverlap_t
...
def pen_plus_solar_curtailoverlap():
    ...
    ###NEW###
    DNK_sp2 = [x[1] for x in solardnk2]
    DNK_wp2 = [x[2] for x in solardnk2]
    DNK_sc2 = list(map(abs,[x[3] for x in solardnk2]))
...
pen_plus_any_curtailoverlap_t("solarcost_w_heat_27_Sept.csv", "solarcost_elec_27_Sept.csv", "solar")

pen_plus_any_poster_curtailoverlap_t("solarcost_w_heat_27_Sept.csv", "solarcost_elec_27_Sept.csv", "solar")
pen_plus_any_poster_curtailoverlap_t("windcost_heatelec_4_Oct.csv", "windcost_elec_28_Sept.csv", "wind")
pen_plus_any_poster_curtailoverlap_t("battcost_heatelec_4_Oct.csv", "battcost_elec_28_Sept.csv", "batt")


def pen_plus_wind_curtailoverlap_t(w_heat, wo_heat):
    '''This makes a 2x2 grid of two axes each showing resource penetration and solar curtailment vs.
    a scaling log of solar. It is very long. It uses gridspec to order the axes, and other than that
    it is about the same as the other pen_plus_curtail() functions. 17/1
    ...
    #plt.rcParams.update({'font.size': 14})
    #plt.rcParams['hatch.linewidth'] = 1
    winddnk = pd.read_csv("results/csvs/Denmark/" + w_heat)
    windesp = pd.read_csv("results/csvs/Spain/"+ w_heat)
    windcol = pd.read_csv("results/csvs/CO/"+ w_heat)
    windcal = pd.read_csv("results/csvs/CA/"+ w_heat)

    winddnk2 = pd.read_csv("results/csvs/Denmark/" + wo_heat)
    windesp2 = pd.read_csv("results/csvs/Spain/"  + wo_heat)
    windcol2 = pd.read_csv("results/csvs/CO/" + wo_heat)
    windcal2 = pd.read_csv("results/csvs/CA/" + wo_heat)
    ...
    ####DENMARK###
    w_cost = winddnk['wind_cost']
    w_cost = w_cost/ 10**6 /0.08442684282600257
    DNK_sp = winddnk['solar_penetration']
    DNK_wp = winddnk['wind_penetration']
    DNK_sc = winddnk['s_curtailment'].abs()

    #NEW
    DNK_sp2 = winddnk2['solar_penetration']
    DNK_wp2 = winddnk2['wind_penetration']
    DNK_sc2 = winddnk2['s_curtailment'].abs()

    ESP_sp2 = windesp2['solar_penetration']
    ESP_wp2 = windesp2['wind_penetration']
    #ESP_sc2 = list(map(abs,[x[4] for x in windesp2]))

    COL_sp2 = windcol2['solar_penetration']
    COL_wp2 = windcol2['wind_penetration']
    #COL_wc2 = list(map(abs,[x[4] for x in windcol2]))

    CAL_sp2 = windcal2['solar_penetration']
    CAL_wp2 = windcal2['wind_penetration']
    ...
    axden1.stackplot(w_cost, DNK_sp2, DNK_wp2, colors = ["#f1c232","#2986cc"], labels = ["Solar\nelec", "Wind\nelec"])
    axden1.stackplot(w_cost, DNK_sp, DNK_wp, colors = ["#f1c232","#2986cc"], labels = ["Solar\nelec + heat", "Wind\nelec + heat"], alpha = 0.5)

    
    axden1.set_ylim(0, 1)
    axden0.set_ylim(0, 1)

    axden1.set_ylabel("Penetration", fontsize = 16)
    #axden1.set_xlabel("Percent flexible source")
    ...
    ESP_sp = windesp['solar_penetration']
    ESP_wp = windesp['wind_penetration']
    #ESP_wc = list(map(abs,[x[4] for x in windesp]))
    ...
    COL_sp = windcol['solar_penetration']
    COL_wp = windcol['wind_penetration']
    #COL_wc = list(map(abs,[x[4] for x in windcol]))
    ...
    axcol1.set_ylabel("Penetration", fontsize = 16)
    ...
    CAL_sp = windcal['solar_penetration']
    CAL_wp = windcal['wind_penetration']
    #CAL_wc = list(map(abs,[x[4] for x in windcal]))
...
pen_plus_wind_curtailoverlap_t("windcost_heatelec_4_Oct.csv", "windcost_elec_28_Sept.csv")
...
def pen_plus_batt_curtailoverlap():
    ...
    plt.rcParams['hatch.linewidth'] = 1
    fig = plt.figure(figsize=(10, 9))
    outer = gridspec.GridSpec(3, 2, wspace=0.2, hspace=0.2, height_ratios = [0.05,1, 1])
    inner_dnk = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=outer[2], wspace=0.1, hspace=0, height_ratios = [0, 2])
    inner_esp = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=outer[3], wspace=0.1, hspace=0, height_ratios = [0, 2])
    inner_col = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=outer[4], wspace=0.1, hspace=0, height_ratios = [0, 2]) #(gridspec(3,1)) to add some extra space
    inner_cal = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=outer[5], wspace=0.1, hspace=0, height_ratios = [0, 2])
    ...
    axden1.stackplot(b_cost, DNK_sp2, DNK_wp2, colors = ["#f1c232","#2986cc"], labels = ["Solar\nelec", "Wind\nelec"])
    axden1.stackplot(b_cost, DNK_sp, DNK_wp, colors = ["#f1c232","#2986cc"], labels = ["Solar\nelec + heat", "Wind\nelec + heat"], alpha = 0.5)
    ...
    fig.legend(lines1, labels1, bbox_to_anchor=(0.88, 0.99), ncol=4)
    fig.text(0.88, 0.08, "€/Wp")
